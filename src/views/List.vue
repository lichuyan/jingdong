<template>
    <div class="panelsbox">
        <cube-scroll class="leftpanels">
            <ul>
                <li v-for="(list,index) in  tabslabel" @click="selected(index)" :class="list.active?'active':''" :key="index">
                    {{list.label}}
                </li>
            </ul>
        </cube-scroll>
	...
	export default {
        ...
        methods:{
	        ...
	        async getclassify(index){
		        try {
			        const result= await  this.$http.get('/api/classify',{params:{type:index}}); // http返回的是promise对象
